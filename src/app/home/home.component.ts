...
selectedfile;
 base64textString:String="";
  public post : FirebaseListObservable<any>;
...
public xo=2;
  ...
  constructor(public afService: AngularFire ,private alertCenterService: AlertCenterService,private router: Router) {
    this.afService.auth.subscribe(
      (auth) => {
        if(auth == null) {
          console.log("Not Logged in. 2 home");
    ...
    this.post.subscribe(res => {this.loader=false; this.amie_count(this.nb_amie);});


    this.postname= afService.database.list('/registeredUsers/vdulAOHw2HVo652S7DMEZCX9c0i2/');
...
var xxx=firebase.storage().ref('/users').child(auth.uid+'/'+x+this.selectedfile.name);
         ...
         cloudRef.on('state_changed',function (snapshot) {
           ...
           if(loader[0]!=100)
             loader[0]=loader[0]+5;
           document.getElementById('kkk').innerHTML = loader[0]+"%";
